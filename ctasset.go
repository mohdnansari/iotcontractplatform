...
func (c *AssetClass) DeleteAllAssets(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	if err != nil {
		...
		/* nihal: query iterator upgrade July 2018*/
	...
	for iter.HasNext() {
		...
		var state Asset
		err = json.Unmarshal([]byte(queryResponse.Value), &state)
		if err != nil {
		...
		if state.Filter(filter) {
			...
			if err != nil {
				err = fmt.Errorf("DeleteAllAssets removeOneAssetFromWorldState for asset %s failed: %s", queryResponse.Key, err)
				log.Errorf(err.Error())
				return nil, err
			}
		}
...
func (c *AssetClass) ReadAsset(stub shim.ChaincodeStubInterface, args []string) ([]byte, error){
	...
	fmt.Println("$$$printing asset value from inside ReadAsset")
...
func (c AssetClass) ReadAllAssetsUnmarshalled(stub shim.ChaincodeStubInterface, args []string) (AssetArray, error) {
	fmt.Println("####$$$$$$$$$$$$>>>>>> In ReadAllAssetsUnmarshalled")
	...
	for iter.HasNext() {
		...
		err = json.Unmarshal([]byte(queryResponse.Value), state)
		if err != nil {
			err = fmt.Errorf("readAllAssetsUnmarshalled unmarshal %s failed: %s", queryResponse.Key, err)
			log.Errorf(err.Error())
			return nil, err
		}
		if state.Filter(filter) {
...
func RegisterDefaultRoutes() {
	...
	fmt.Println("####$$$$$$$$$$$$>>>>>> In RegisterDefaultRoutes")
