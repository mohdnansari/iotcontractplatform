...
func AddRoute(functionName string, method string, class AssetClass, function ChaincodeFunc) error {
	...
	fmt.Println("In AddRoute, adding the following function")
	fmt.Println("!!!!!!!>>>FunctionName=" + functionName + ", method=" + method)
...
func getDeployFunctions() []ChaincodeFunc {
	fmt.Println("$$$$$$$$$$$$$$$$>>>>>>>> In getDeployFunctions()")
...
//return nil, nil
	...
	//change by nihal: skip arguments check and hardcode the init parameters as this code is v0.6 compliant
	//iargs[0] = args[0]
	//iargs[1] = ContractVersion
	iargs[0] = "{\"version\":\"0.1\",\"nickname\":\"IOTCPSample101\"}"
    iargs[1] = ContractVersion
...
func Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	function, args := stub.GetFunctionAndParameters()
	//if this is a query request route to Query method
	if (function == "readAllAssets") || (function == "readAsset") || (function == "readAllRoutes") || (function == "readAssetSamples") || (function == "readAssetSchemas") || (function == "readRecentStates") || (function == "readAssetStateHistory") || (function == "readContractState") || (function == "readWorldState") {
			return Query(stub)
	}
	...
	if len(eventToReportBytes) == 0 {
		...
		if err != nil {
			err := fmt.Errorf("Invoke (%s) failed to marshal returned event to report with error %s, remember that chaincode events should be maps. eventToReportBytes=>%s<, args[0]=>%s<", function, err,string(eventToReportBytes),args[0])
			log.Error(err)
			setStubEvent(stub, err, nil)
			//return nil, err
			return shim.Error(err.Error())
		...
		fmt.Println("$$$printing asset value from inside Invoke")
	fmt.Println(string(eventToReportBytes))
	}
	...
	//return nil, nil
	//return shim.Success(nil)
	return shim.Success(eventToReportBytes)
}
...
func Query(stub shim.ChaincodeStubInterface) pb.Response {
	...
	fmt.Println("$$$printing asset value from inside Query")
